...
object processamento_spark
{
    ...
    val dolar = spark.read.format("csv").option("header", "true").option("delimiter", ",").load("/user/lucasSampaio/dolar_*.csv");
    val crypto = spark.read.format("csv").option("header", "true").option("delimiter",",").load("/user/lucasSampaio/crypto_*.csv");
